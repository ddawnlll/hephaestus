...
- **Praxis Truth Kernel** (`ddawnlll/praxis`) is the independent verification layer — called via `tools/praxis-bridge.sh`
...
5. **Fail-closed** — gate scripts reject on error, never "pass silently"
...
8. **Praxis before T1** — Truth Kernel runs before any LLM is woken for gate review
...
| `templates/prompts/` | Orchestrator tick, worker_task prompts |
| `tools/praxis/` (submodule) | `ddawnlll/praxis` — Truth Kernel CLI |
| `tools/praxis-bridge.sh` | Thin wrapper that calls `praxis verify` |
